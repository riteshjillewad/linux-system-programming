...
int main()
{
    ...
    handle = dlopen("./question4_server.so", RTLD_LAZY);
    if(handle == NULL)
    {
    ...
    // Multiplication (ERROR MUST OCCUR, AS THIS LIBRARY DOES NOT EXISTS)
    ...
    // Division (ERROR MUST OCCUR, AS THIS LIBRARY DOES NOT EXISTS)
